...
class ParserGenerator:
    ...
    def make_dfa(self, start: "NFAState",
    ...
    def dump_nfa(self, name: str, start: "NFAState",
    ...
    def parse_alt(self) -> tuple["NFAState", "NFAState"]:
        ...
        a, b = self.parse_item()
        while self.value in (
                "(", "[") or self.type in (token.NAME, token.STRING):
            c, d = self.parse_item()
            b.addarc(c)
            b = d
        return a, b
    ...
    def parse_atom(self) -> tuple["NFAState", "NFAState"]:
        ...
        if self.value == "(":
            ...
            self.raise_error(
                f"expected (...) or NAME or STRING, got {
                    self.type}/{
                    self.value}"
            )
    ...
    def expect(self, type: int, value: Optional[Any] = None) -> str:
        if self.type != type or (value is not None and self.value != value):
            self.raise_error(
                f"expected {type}/{value}, got {self.type}/{self.value}")
        value = self.value
        self.gettoken()
        return value
