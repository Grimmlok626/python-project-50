...
def tokenize(source: str,
             grammar: Optional[Grammar] = None) -> Iterator[TokenInfo]:
    lines = source.split("\n")
    ...
    try:
        for token in pytokens.tokenize(source):
            ...
            if token.type == TokenType.identifier and token_str in (
                    "async", "await"):
                # Black uses `async` and `await` token types just for those two
                # keywords
                yield (
        ...
        raise TokenError(
            "Unexpected EOF in multi-line statement", (line, column))
    except pytokens.TokenizeError as exc:
        raise TokenError(
            f"Failed to parse: {
                type(exc).__name__}", (line, column))
