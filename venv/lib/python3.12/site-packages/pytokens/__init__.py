...
class TokenIterator:
    ...
    fstring_prefix_quote_stack: list[tuple[str, str]] = field(
    ...
    def match(self, *options: str, ignore_case: bool = False) -> bool:
        for option in options:
            if self.current_index + len(option) > len(self.source):
                continue
            snippet = self.source[self.current_index:
                                  self.current_index + len(option)]
            if ignore_case:
                option = option.lower()
    ...
    def decimal(self) -> Token:
        ...
        while self.is_in_bounds() and (
            ...
            self.advance()
        # Before advancing over the 'e', ensure that there has been at least 1
        # digit before the 'e'
        if self.current_index + 1 < len(self.source) and (
            ...
            self.advance()
            self.advance()
            # optional third advance not necessary as itll get advanced just
            # below
    ...
    def find_opening_quote(self) -> int:
        # Quotes should always be within 3 chars of the beginning of the string
        # token
            ...
            while self.is_in_bounds():
                ...
                if char == "{":
                    if self.peek_next() == "{":
                        ...
                        # If fstring-middle is empty, skip it by returning the
                        # next step token
                ...
                if self.match(self.fstring_quote):
                    ...
                    # If fstring-middle is empty, skip it by returning the next
                    # step token
                    if self.current_index == start_index:
                        return self.fstring()
    ...
    def indent(self) -> Token:
        ...
        current_indent = "" if len(
            self.indent_stack) == 0 else self.indent_stack[-1]
            ...
            while self.is_in_bounds():
                if self.is_whitespace():
                    ...
                    if self.all_whitespace_on_this_line:
                        ...
                        self.next_line()
                        # Preserve this boolean, we're on the same line
                        # semantically
                        self.all_whitespace_on_this_line = False
...
def tokenize(
    ...
    *,
    ...
    token_iterator = TokenIterator(
        source, issue_128233_handling=issue_128233_handling)
    if fstring_tokens:
