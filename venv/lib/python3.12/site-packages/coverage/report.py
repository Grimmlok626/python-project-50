...
class SummaryReporter:
    ...
    def __init__(self, coverage: Coverage) -> None:
        ...
        if self.output_format not in {"text", "markdown", "total"}:
            raise ConfigError(
                f"Unknown report format choice: {
    ...
    def report_text(
        ...
        max_n = max(
            len(total_line[header.index("Cover")]) + 2, len(" Cover")) + 1
        max_n = max([max_n] +
                    [len(line[header.index("Cover")]) +
                     2 for line in lines_values])
        formats = dict(
            Name="{:{name_len}}",
            Stmts="{:>7}",
            Miss="{:>7}",
        ...
        for values in lines_values:
            self.write_items(
                (
                    formats[item].format(
                        str(value), name_len=max_name, n=max_n - 1)
    ...
    def report_markdown(
        ...
        """
        # Prepare the formatting strings, header, and column sorting.
        max_name = max((len(line[0].replace("_", "\\_"))
                       for line in lines_values), default=0)
        ...
        for item, value in zip(header, total_line):
            if value == "":
                ...
                insert = f" **{value}**"
            total_line_items += formats[item].format(
    ...
    def report(self, morfs: Iterable[TMorf] | None,
               outfile: IO[str] | None = None) -> float:
