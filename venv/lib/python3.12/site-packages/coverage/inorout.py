...
class InOrOut:
            ...
            for pkg in self.source_pkgs:
                ...
                if modfile:
                    if self.third_match.match(modfile):
                        _debug(
                            f"Source in third-party: source_pkg {
                                pkg!r} at {
                                modfile!r}",
                        )
                    ...
                    for pathdir in path:
                        if self.third_match.match(pathdir):
                            _debug(
                                f"Source in third-party: {
                                    pkg!r} path directory at {
                                    pathdir!r}",
                            )
    ...
    def should_trace(self, filename: str, frame: FrameType |
                     None = None) -> TFileDisposition:
        ...
        if not disp.has_dynamic_filename:
            if not disp.source_filename:
                raise PluginError(
                    f"Plugin {
                        plugin!r} didn't set source_filename for '{
    ...
    def check_include_omit_etc(self, filename: str,
                               frame: FrameType | None) -> str | None:
        ...
        if self.source_match or self.source_pkgs_match:
            ...
            if self.third_match.match(
                    filename) and not self.source_in_third_match.match(filename):
        ...
        if self.include or self.source_dirs or self.source_pkgs:
            ...
            for mod in list(sys.modules.values()):
                ...
                if len(getattr(mod, "__path__", ())) > 1:
                    # A namespace package, which confuses this code, so ignore
                    # it.
                    continue
    ...
    def _warn_about_unmeasured_code(self, pkg: str) -> None:
        ...
        if mod is None:
            self.warn(
                f"Module {pkg} was never imported.",
        ...
        if not module_has_file(mod):
            self.warn(
                f"Module {pkg} has no Python source.",
    ...
    def find_possibly_unexecuted_files(
            self) -> Iterable[tuple[str, str | None]]:
    ...
    def _find_executable_files(
            self, src_dir: str) -> Iterable[tuple[str, str | None]]:
