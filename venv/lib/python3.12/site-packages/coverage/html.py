        ...
        for lineno, tokens in enumerate(fr.source_token_lines(), start=1):
            ...
            if category and self.config.show_contexts:
                contexts = human_sorted(
                    c or self.EMPTY for c in contexts_by_lineno.get(
                        lineno, ()))
...
class HtmlReporter:
    ...
    def __init__(self, cov: Coverage) -> None:
        ...
        self.template_globals = {
            ...
            # These css classes determine which lines are highlighted by
            # default.
            "category": {
                "exc": "exc show_exc",
                "mis": "mis show_mis",
        ...
        self.index_tmpl = Templite(
            read_data("index.html"),
            self.template_globals)
        ...
        for ftr in files_to_report:
            ...
            for noun, plural_noun in ftr.fr.code_region_kinds():
                if noun not in self.index_pages:
                    self.index_pages[noun] = self.new_index_page(
                        noun, plural_noun)
    ...
    def write_html_page(self, ftr: FileToReport) -> None:
        ...
        context_codes = {
            y: i for (
                i, y) in enumerate(
                x[0] for x in contexts.most_common())}
        if context_codes:
        ...
        for ldata in file_data.lines:
            ...
            if ldata.long_annotations:
                ...
                assert len(longs) == 1, (
                    f"Had long annotations in {
                        ftr.fr.relative_filename()}: {longs}"
                )
    ...
    def write_file_index_page(self, first_html: str, final_html: str) -> None:
        ...
        print_href = stdout_link(index_file,
    ...
    def write_region_index_pages(
            self, files_to_report: Iterable[FileToReport]) -> None:
...
class IncrementalChecker:
    ...
    def can_skip_file(self, data: CoverageData,
                      fr: FileReporter, rootname: str) -> bool:
