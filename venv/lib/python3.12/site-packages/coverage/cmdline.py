...
class CoverageOptionParser(optparse.OptionParser):
    ...
    def parse_args_ok(
            self, args: list[str]) -> tuple[bool, optparse.Values | None, list[str]]:
...
class CoverageScript:
    ...
    def command_line(self, argv: list[str]) -> int:
        ...
        if options.action == "debug":
            ...
            self.coverage.combine(
                data_paths, strict=True, keep=bool(
                    options.keep))
            self.coverage.save()
            return OK
    ...
    def do_signal_save(self, _signum: int,
    ...
    def do_run(self, options: optparse.Values, args: list[str]) -> int:
        ...
        if not args:
            if options.module:
                ...
                return ERR
            command_line = cast(
                str, self.coverage.get_option("run:command_line"))
        ...
        if options.concurrency == "multiprocessing":
            ...
            # multiprocessing.
            for opt_name in ["branch", "include",
                             "omit", "pylib", "source", "timid"]:
    ...
    def do_debug(self, args: list[str]) -> int:
        ...
        if args[0] == "sys":
            ...
            write_formatted_info(
                print, "config", self.coverage.config.debug_info())
        elif args[0] == "premain":
...
def main(argv: list[str] | None = None) -> int | None:
    ...
    try:
        ...
        traceback.print_exception(
            *err.args)  # pylint: disable=no-value-for-parameter
        status = ERR
