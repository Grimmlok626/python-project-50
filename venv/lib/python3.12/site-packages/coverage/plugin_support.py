...
class Plugins:
    ...
    def load_from_config(
        ...
        for module in modules:
            ...
            if not coverage_init:
                raise PluginError(
                    f"Plugin module {
                        module!r} didn't define a coverage_init function",
                )
    ...
    def _add_plugin(
        ...
        if self.debug and self.debug.should("plugin"):
            self.debug.write(
                f"Loaded plugin {
                    self.current_module!r}: {
                    plugin!r}")
            labelled = LabelledDebug(
                f"plugin {
                    self.current_module!r}",
                self.debug)
...
class LabelledDebug:
    ...
    def __init__(self, label: str, debug: TDebugCtl,
                 prev_labels: Iterable[str] = ()):
...
class DebugPluginWrapper(CoveragePlugin):
    ...
    def find_executable_files(self, src_dir: str) -> Iterable[str]:
        ...
        self.debug.write(
            f"find_executable_files({src_dir!r}) --> {executable_files!r}")
...
class DebugFileTracerWrapper(FileTracer):
    ...
    def dynamic_source_filename(self, filename: str,
                                frame: FrameType) -> str | None:
    ...
    def line_number_range(self, frame: FrameType) -> tuple[TLineNo, TLineNo]:
        ...
        self.debug.write(
            f"line_number_range({self._show_frame(frame)}) --> {pair!r}")
        return pair
...
class DebugFileReporterWrapper(FileReporter):
    ...
    def __init__(self, filename: str, reporter: FileReporter,
                 debug: LabelledDebug) -> None:
