...
class PythonParser:
    ...
    def _raw_parse(self) -> None:
        ...
        for toktype, ttext, (slineno, _), (elineno, _), ltext in tokgen:
            ...
            if ttext.strip() and toktype != tokenize.COMMENT:
                ...
                empty = False
                if not first_line:
                    # The token is not white space, and is the first in a
                    # statement.
                    first_line = slineno
        ...
        for node in ast.walk(self._ast_root):
            ...
            if isinstance(node, (ast.ClassDef, ast.FunctionDef,
                ...
                first_line = min(
                    ...
                    default=node.lineno)
                if self.excluded.intersection(
                        range(first_line, node.lineno + 1)):
    ...
    def translate_arcs(self, arcs: Iterable[TArc]) -> set[TArc]:
        ...
        return {(self.first_line(a), self.first_line(b))
    ...
    def exit_counts(self) -> dict[TLineNo, int]:
        ...
        for l1, l2 in self.arcs():
            assert l1 > 0, f"{
                l1=} should be greater than zero in {
                self.filename}"
    ...
    def missing_arc_description(self, start: TLineNo, end: TLineNo) -> str:
        ...
        fragment_pairs = self._missing_arc_fragments.get(
            (start, end), [(None, None)])
...
class Block:
    ...
    def process_break_exits(
            self, exits: set[ArcStart], add_arc: TAddArcFn) -> bool:
        """Process break exits."""
        return False

    def process_continue_exits(
            self, exits: set[ArcStart], add_arc: TAddArcFn) -> bool:
        """Process continue exits."""
        return False

    def process_raise_exits(
            self, exits: set[ArcStart], add_arc: TAddArcFn) -> bool:
        """Process raise exits."""
        return False

    def process_return_exits(
            self, exits: set[ArcStart], add_arc: TAddArcFn) -> bool:
...
class TryBlock(Block):
    ...
    def __init__(self, handler_start: TLineNo | None,
                 final_start: TLineNo | None) -> None:
...
class AstArcAnalyzer:
    ...
    def __init__(
        ...
        self.missing_arc_fragments: TArcFragments = collections.defaultdict(
            list)
    ...
    def analyze(self) -> None:
        ...
        for node in ast.walk(self.root_node):
            ...
            code_object_handler = getattr(
                self, f"_code_object__{node_name}", None)
    ...
    def with_jump_fixers(self) -> dict[TArc, tuple[TArc, TArc]]:
        ...
        for start in self.all_with_starts:
            ...
            if not nexts:
                continue
            assert len(
                nexts) == 1, f"Expected one arc, got {nexts} with {start =}"
            nxt = nexts.pop()
    ...
    def add_arc(
        ...
        if self.debug:  # pragma: debugging
            print(
                f"Adding possible arc: ({start}, {end}): {
                    missing_cause_msg!r}, {
                    action_msg!r}")
        ...
        if handler is not None:
            ...
            if env.TESTING:
                if node_name not in self.OK_TO_DEFAULT:
                    raise RuntimeError(
                        f"*** Unhandled: {node}")  # pragma: only failure
    ...
    def _handle__Break(self, node: ast.Break) -> set[ArcStart]:
        ...
        break_start = ArcStart(
            here, cause="the break on line {lineno} wasn't executed")
    ...
    def _handle__For(self, node: ast.For) -> set[ArcStart]:
        ...
        from_start = ArcStart(
            start, cause="the loop on line {lineno} never started")
        ...
        from_start = ArcStart(
            start, cause="the loop on line {lineno} didn't complete")
        if node.orelse:
    ...
    if sys.version_info >= (3, 10):
        ...
        def _handle__Match(self, node: ast.Match) -> set[ArcStart]:
            ...
            while isinstance(
                    pattern, ast.MatchAs) and pattern.pattern is not None:
                pattern = pattern.pattern
            had_wildcard = (
                isinstance(
                    pattern,
                    ast.MatchAs) and pattern.pattern is None and case.guard is None  # pylint: disable=undefined-loop-variable
            )
            ...
            if not had_wildcard:
                exits.add(
                    ArcStart(
                        case_start,
                        cause="the pattern on line {lineno} always matched"),
                )
            return exits
    ...
    def _handle__Raise(self, node: ast.Raise) -> set[ArcStart]:
        here = self.line_for_node(node)
        raise_start = ArcStart(
            here, cause="the raise on line {lineno} wasn't executed")
    ...
    def _handle__Try(self, node: ast.Try) -> set[ArcStart]:
        ...
        if node.finalbody:
            ...
            final_exits = self.process_body(
                node.finalbody, prev_starts=final_from)
    ...
    def _handle__With(self, node: ast.With) -> set[ArcStart]:
        if env.PYBEHAVIOR.exit_with_through_ctxmgr:
            starts = [self.line_for_node(item.context_expr)
