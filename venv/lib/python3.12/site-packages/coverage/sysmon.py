        ...
        def _decorator(method: AnyCallable) -> AnyCallable:
            ...
            def _wrapped(self: Any, *args: Any) -> Any:
                try:
                    ...
                    # log(f" end {id(self):#x}:{method.__name__}({',
                    # '.join(args_reprs)})")
                    return ret
...
class SysMonitor(Tracer):
    ...
    def start(self) -> None:
        ...
        with self.lock:
            ...
            register = functools.partial(
                sys_monitoring.register_callback, self.myid)
    ...
    @panopticon("code", "@")
    def sysmon_py_start(self, code: CodeType,
                        instruction_offset: TOffset) -> MonitorReturn:
        ...
        if tracing_code is None:
            ...
            if disp is None:
                ...
                disp = self.should_trace(
                    filename, frame)  # type: ignore[arg-type]
    ...
    def sysmon_line_lines(self, code: CodeType,
                          line_number: TLineNo) -> MonitorReturn:
    ...
    @panopticon("code", "line")
    def sysmon_line_arcs(self, code: CodeType,
                         line_number: TLineNo) -> MonitorReturn:
    ...
    def sysmon_branch_either(
        ...
        # Re-map the destination offset through always-jumps to deal with NOP
        # etc.
        dests = {destination_offset}
        while (dest := code_info.always_jumps.get(
                destination_offset)) is not None:
