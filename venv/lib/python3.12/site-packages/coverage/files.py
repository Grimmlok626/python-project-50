...
class ModuleMatcher:
    ...
    def __init__(
            self, module_names: Iterable[str], name: str = "unknown") -> None:
...
class PathAliases:
    ...
    def add(self, pattern: str, result: str) -> None:
        ...
        if not self.relative:
            if not pattern.startswith(
                    "*") and not isabs_anywhere(pattern + pattern_sep):
    ...
    def map(self, path: str, exists: Callable[[
            str], bool] = source_exists) -> str:
            ...
            if len(parts) > 1:
                ...
                if not any(p == pattern for p, _, _ in self.aliases):
                    self.debugfn(
                        f"Generating rule: {
                            pattern!r} -> {
                            result!r} using regex {
                            regex_pat!r}",
                    )
                    self.aliases.append(
                        (pattern, re.compile(regex_pat), result))
...
def find_python_files(
        dirname: str, include_namespace_packages: bool) -> Iterable[str]:
