...
def _patch_execv(cov: Coverage, config: CoverageConfig,
    ...
    def make_execv_patch(fname: str, old_execv: Any) -> Any:
        def coverage_execv_patch(*args: Any, **kwargs: Any) -> Any:
            ...
            if fname.endswith("e"):
                ...
                if env.TESTING:
                    # The subprocesses need to use the same core as the main
                    # process.
                    ...
                    if (cache_prefix := os.getenv(
                            "PYTHONPYCACHEPREFIX")) is not None:
...
def _patch_subprocess(config: CoverageConfig, debug: TDebugCtl,
                      make_pth_file: bool) -> None:
