...
class PyTracer(Tracer):
    ...
    def __init__(self) -> None:
        ...
        self.data_stack: list[tuple[TTraceFileData |
                                    None, str | None, TLineNo, bool]] = []
    ...
    def _trace(
        ...
        if self.stopped and sys.gettrace(
        ) == self._cached_bound_method_trace:  # pylint: disable=comparison-with-callable
        ...
        if event == "call":
            ...
            if self.should_start_context and self.context is None:
                context_maybe = self.should_start_context(
                    frame)  # pylint: disable=not-callable
                if context_maybe is not None:
                    self.context = context_maybe
                    started_context = True
                    assert self.switch_context is not None
                    self.switch_context(
                        self.context)  # pylint: disable=not-callable
                else:
            ...
            if self.cur_file_data is not None:
                ...
                if self.trace_arcs:
                    cast(
                        set_TArc, self.cur_file_data).add(
            ...
            if self.trace_arcs and self.cur_file_data:
                ...
                if RESUME is not None:
                    if len(code) == lasti + 2:
                        # A return from the end of a code object is a real
                        # return.
                        real_return = True
                    else:
    ...
    def stop(self) -> None:
        ...
        suppress_warning = (
            env.PYPY and self.in_atexit and tf is None) or env.METACOV
