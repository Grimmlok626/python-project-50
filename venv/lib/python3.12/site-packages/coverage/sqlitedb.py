...
class SqliteDb:
    ...
    def __init__(self, filename: str, debug: TDebugCtl,
                 no_disk: bool = False) -> None:
        self.debug = debug
    ...
    def _connect(self) -> None:
        ...
        try:
            ...
            if self.filename.startswith("file:"):
                self.con = sqlite3.connect(
                    self.filename, check_same_thread=False, uri=True)
            else:
                self.con = sqlite3.connect(
                    self.filename, check_same_thread=False)
        except sqlite3.Error as exc:
            raise DataError(
                f"Couldn't use data file {
                    self.filename!r}: {exc}") from exc

        if self.debug.should("sql"):
            self.debug.write(f"Connected to {self.filename!r} as {self.con!r}")

        self.con.create_function(
            "REGEXP", 2, lambda txt, pat: re.search(
                txt, pat) is not None)
        ...
        # This pragma makes writing faster. It disables rollbacks, but we never
        # need them.
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        ...
        if self.nest == 0:
            try:
                ...
                if self.debug.should("sql"):
                    self.debug.write(
                        f"EXCEPTION from __exit__: {
                            exc_one_line(exc)}")
                raise DataError(
                    f"Couldn't end data file {
                        self.filename!r}: {exc}") from exc
    ...
    def execute_void(
            self, sql: str, parameters: Iterable[Any] = (), fail_ok: bool = False) -> None:
    ...
    def execute_for_rowid(self, sql: str,
                          parameters: Iterable[Any] = ()) -> int:
    ...
    def execute_one(
            self, sql: str, parameters: Iterable[Any] = ()) -> tuple[Any, ...] | None:
