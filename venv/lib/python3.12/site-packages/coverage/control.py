...
class Coverage(TConfigurable):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        # If we're invoked from a .pth file, we shouldn't try to make another
        # one.
        ...
        self._plugin_override = cast(
            Union[Iterable[TCoverageInit], None], plugins)
    ...
    def _write_startup_debug(self) -> None:
        ...
        with self._debug.without_callers():
            ...
            if self._debug.should("pybehave"):
                write_formatted_info(
                    self._debug.write, "pybehave", env.debug_info())
                wrote_any = True

        if wrote_any:
            write_formatted_info(self._debug.write, "end", ())

    def _should_trace(self, filename: str,
                      frame: FrameType) -> TFileDisposition:
    ...
    def _warn(self, msg: str, slug: str | None = None,
    ...
    def set_option(self, option_name: str, value: TConfigValueIn |
                   TConfigSectionIn) -> None:
    ...
    def load(self) -> None:
        ...
        should_skip = self.config.parallel and not os.path.exists(
    ...
    def _init_for_start(self) -> None:
        ...
        if not dycon or dycon == "none":
            ...
            raise ConfigError(
                f"Don't understand dynamic_context setting: {
                    dycon!r}")
        ...
        if self.config.sigterm:
            is_main = (threading.current_thread() ==
                       threading.main_thread())  # fmt: skip
    ...
    def start(self) -> None:
        ...
        apply_patches(self, self.config, self._debug,
                      make_pth_file=self._make_pth_file)
    ...
    def _on_sigterm(self, signum_unused: int,
                    frame_unused: FrameType | None) -> None:
    ...
    def _make_aliases(self) -> PathAliases:
        ...
        aliases = PathAliases(
            debugfn=(self._debug.write if self._debug.should(
                "pathmap") else None),
    ...
    def analysis(
            self, morf: TMorf) -> tuple[str, list[TLineNo], list[TLineNo], str]:
    ...
    def _get_file_reporters(
        ...
        morfs = sorted(
            morfs, key=lambda m: m if isinstance(
                m, str) else m.__name__)
    ...
    def _prepare_data_for_reporting(self) -> None:
        ...
        if self.config.paths:
            mapped_data = CoverageData(
                warn=self._warn, debug=self._debug, no_disk=True)
            if self._data is not None:
                mapped_data.update(
                    self._data, map_path=self._make_aliases().map)
    ...
    def xml_report(
        ...
        with override_config(
            ...
            return render_report(self.config.xml_output,
                                 XmlReporter(self), morfs, self._message)
    ...
    def json_report(
        ...
        with override_config(
            ...
            return render_report(self.config.json_output,
                                 JsonReporter(self), morfs, self._message)
    ...
    def lcov_report(
        ...
        with override_config(
            ...
            return render_report(self.config.lcov_output,
    ...
    def sys_info(self) -> Iterable[tuple[str, Any]]:
        ...
        info = [
            ...
            ("core", self._collector.tracer_name()
            ...
            ("plugins.context_switchers", plugin_info(
                self._plugins.context_switchers)),
            ...
            (
                "config_contents",
                repr(
                    self.config._config_contents) if self.config._config_contents else "-none-",
            ),
            ...
            ("cwd", os.getcwd()),
            ("path", sys.path),
            ("environment", [f"{k} = {v}" for k,
             v in relevant_environment_display(os.environ)]),
