...
class CoverageData:
    ...
    * **file tracer names**: the module names of the file tracer plugins that
    ...
    def _choose_filename(self) -> None:
        ...
        if self._no_disk:
            self._filename = f"file:coverage-{
                uuid.uuid4()}?mode=memory&cache=shared"
        else:
    ...
    def _open_db(self) -> None:
        ...
        self._dbs[threading.get_ident()] = SqliteDb(
            self._filename, self._debug, self._no_disk)
        self._read_db()
    ...
    def _choose_lines_or_arcs(self, lines: bool = False,
    ...
    def touch_files(
            self, filenames: Collection[str], plugin_name: str | None = None) -> None:
    ...
    def update(
        ...
        with self._connect() as con:
            ...
            con.execute_void("ATTACH DATABASE ? AS other_db",
                             (other_data.data_filename(),))

            # Create temporary table with mapped file paths to avoid repeated
            # map_path() calls
            con.execute_void("""
            ...
            # Insert tracers from other_db (avoiding conflicts we already
            # checked)
            con.execute_void("""
