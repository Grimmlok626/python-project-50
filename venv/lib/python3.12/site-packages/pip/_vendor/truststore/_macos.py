...
if _mac_version_info < (10, 8):
    raise ImportError(
        f"Only OS X 10.8 and newer are supported, not {
            _mac_version_info[0]}.{
            _mac_version_info[1]}"
    )
...
def _handle_osstatus(result: OSStatus, _: typing.Any,
...
def _verify_peercerts_impl_macos_10_13(
    ...
    Security.SecTrustEvaluate(
        sec_trust_ref,
        ctypes.byref(sec_trust_result_type))
...
def _verify_peercerts_impl_macos_10_14(
    ...
    if sec_trust_eval_result == 1:
        ...
        raise ssl.SSLError(
            f"Unknown result from Security.SecTrustEvaluateWithError: {
                sec_trust_eval_result!r}"
        )
