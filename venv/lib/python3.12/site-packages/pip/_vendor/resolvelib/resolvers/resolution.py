...
class Resolution(Generic[RT, CT, KT]):
    ...
    def _get_updated_criteria(
            self, candidate: CT) -> dict[KT, Criterion[RT, CT]]:
    ...
    def _backjump(self, causes: list[RequirementInformation[RT, CT]]) -> bool:
        ...
        while len(self._states) >= 3:
            ...
            # Optimistically backtrack to a state that caused the
            # incompatibility
    ...
    def resolve(
            self, requirements: Iterable[RT], max_rounds: int) -> State[RT, CT, KT]:
        if self._states:
        ...
        for round_index in range(max_rounds):
            ...
            satisfied_names = set(
                self.state.criteria.keys()) - set(unsatisfied_names)
            ...
            if not narrowed_unstatisfied_names:
                raise RuntimeError(
                    "narrow_requirement_selection returned 0 names")

            # If there is only 1 unsatisfied name skip calling
